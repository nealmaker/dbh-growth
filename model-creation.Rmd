...
# ridge regression FAILED: RMSE VALUES MISSING
# set.seed(seed)
# fit.ridge <- train(x, y, method = 'ridge', metric=metric,
#                    preProc=c("center", "scale"), trControl=control)
# quantile regression with LASSO penalty FAILED: RMSE VALUES MISSING
# set.seed(seed)
...
fit.knn <- train(x, y, method = 'knn', metric=metric, 
                 preProc=c("center", "scale"), trControl=control)
# bayesian ridge regression FAILED: RMSE VALUES MISSING
# set.seed(seed)
...
# elastic net
set.seed(seed)
fit.enet <- train(x, y, method = 'enet', metric=metric, 
                  preProc=c("center", "scale"), trControl=control)
# linear regression
set.seed(seed)
...
A diverse set of `r length(fits)` algorithms was successfully spot-checked using repeated ten-fold cross validation, with root mean square error as the evaluation metric. Three algorithms failed to train and were discarded. 

---- LASSO & RIDGE ARE FAILING. SHOULD TRY SPP-SPECIFIC VERSIONS w/o ONE_HOT.----
...
```{r spotPredtimes, fig.cap="Prediction times for spot-checked algorithms, based on 30 random samples of 10,000 trees each.", cache=FALSE}
